...
class ECAPAModel_ECG(nn.Module):
    def __init__(self, lr, lr_decay, C, n_class, m, s, test_step, **kwargs):
        ...
        # self.optim = torch.optim.Adam(self.parameters(), lr=lr, weight_decay=2e-5)
        # self.optim = torch.optim.Adam(self.speaker_encoder.parameters(), lr=lr, weight_decay=2e-5)
        self.optim = torch.optim.SGD(self.speaker_encoder.parameters(), lr=lr, weight_decay=2e-5)
        # print(self.speaker_encoder.parameters() == self.parameters())
        # exit()
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optim, step_size=test_step, gamma=lr_decay)
        print(time.strftime("%m-%d %H:%M:%S") + " Model para number = %.2f" % (
                sum(param.numel() for param in self.speaker_encoder.parameters()) / 1024 / 1024))
    ...
    def train_network_withEER(self, epoch, loader):
        ...
        for num, (data1, data_pos, data_neg, labels) in enumerate(loader, start=1):
            # for name, para in self.speaker_encoder.named_parameters():
            # 	print(name)
            # 	print(para.shape, para.requires_grad, para.grad)
            # exit()
            self.optim.zero_grad()
            labels = torch.LongTensor(labels).cuda()
            speaker_embedding = self.speaker_encoder.forward(data1.cuda(), aug=False)
            pos_embedding = self.speaker_encoder.forward(data_pos.cuda(), aug=False)
            neg_embedding = self.speaker_encoder.forward(data_neg.cuda(), aug=False)
            nloss, prec = self.speaker_loss.forward(speaker_embedding, labels)
            # nloss, prec = self.original_loss.forward(speaker_embedding, labels)
            ...
            alpha = 0.8
            triple = trble_loss(anchor=speaker_embedding, positive=pos_embedding, negative=neg_embedding)
    ...
    def train_network_withEER_range(self, epoch, loader):
        ...
        for num, (data1, data_pos, data_neg, labels) in enumerate(loader, start=1):
            ...
            ran = range_loss(anchor=speaker_embedding, positive=pos_embedding, targets=labels)
    ...
    def detect_nan(self, epoch, loader):
        ...
        for num, (data1, data_pos, data_neg, labels) in enumerate(loader, start=1):
            ...
            # exit()
            num += 2
            # torch.nn.utils.clip_grad_norm_(self.speaker_encoder.parameters(), max_norm=1.0)
            self.optim.step()
            index += len(labels)
            top1 += prec
            loss += Loss_all.detach().cpu().numpy()
    ...
    def test_network_acc(self, epoch, loader):
        ...
        for num, (data, labels) in tqdm.tqdm(enumerate(loader, start=1)):
            ...
            st = time.time()
            labels = torch.LongTensor(labels).cuda()
            speaker_embedding = self.speaker_encoder.forward(data, aug=False).cuda()
            nloss, prec = self.speaker_loss.forward(speaker_embedding, labels)
            et = time.time()
            at = et-st
            # nloss, prec = self.original_loss.forward(speaker_embedding, labels)
    ...
    def feature_label(self, loader):
        ...
        for num, (data, labels) in tqdm.tqdm(enumerate(loader, start=1)):
            ...
            labels = torch.LongTensor(labels).cuda()
            speaker_embedding = self.speaker_encoder.forward(data.cuda(), aug=True)
            embed.append(speaker_embedding.detach().cpu().numpy())
